 ...
 */
export default function ChangeProfleCard({theme, lang, type, value, hide, trigger}) {
    // Dispatch to change Redux states
    const dispatch = useDispatch()

    // Profile info
    let { image } = useSelector( (state) => state.profile )
    ...
    const gestureHandler = useAnimatedGestureHandler({
        ...
        onEnd: (event) => {
            if (event.velocityY > windowHeight/3) {
                translateY.value = withTiming(windowHeight).then(tryToHide())
            } else {
    ...
    return(
        <PanGestureHandler onGestureEvent={gestureHandler}>
            <Animated.View style={[PS.animatedProfileChangeCard, animation, {backgroundColor: FetchColor(theme, "DARKER")}]}>

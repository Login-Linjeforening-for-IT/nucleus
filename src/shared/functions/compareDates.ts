 ...
 */
export default function CompareDates ({firstDate, secondDate}: CompareDatesProps): boolean | number {
    if(firstDate != null && secondDate != null){
        // Concatenating start:
        const firstYear   = (firstDate)[0] + (firstDate)[1] + (firstDate)[2] + (firstDate)[3]
        ...
        // Concatenating end:
        const secondYear     = (secondDate)[0]  + (secondDate)[1] + (secondDate)[2] + (secondDate)[3]
